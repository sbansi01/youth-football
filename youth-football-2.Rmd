...
# For Highland Home, AL, the zip code (36041) seems to be a good substitute: https://censusreporter.org/profiles/86000US36041-36041/
census_data_highland_ar_2021 <- get_acs(geography = "zcta",
                           variables = c("B01003_001", "B02001_003", "B19013_001"),
                           year = 2021,
                           output = "wide") %>%
  clean_names() %>%
...
census_data_ma_subdivisions_2021 <- get_acs(geography = "county subdivision",
                           ...
                           state = "MA",
                           output = "wide") %>%
  filter(NAME == "Acton town, Middlesex County, Massachusetts" | NAME == "	
Canton town, Norfolk County, Massachusetts" | NAME == "Concord town, Middlesex County, Massachusetts" | NAME == "Franklin Town city, Norfolk County, Massachusetts" | NAME == "Holliston town, Middlesex County, Massachusetts" | NAME == "Mansfield town, Bristol County, Massachusetts" | NAME == "	
Newbury town, Essex County, Massachusetts" | NAME == "North Andover town, Essex County, Massachusetts" | NAME == "Norwell town, Plymouth County, Massachusetts" | NAME == "	
Paxton town, Worcester County, Massachusetts" | NAME == "Raynham town, Bristol County, Massachusetts" | NAME == "Sudbury town, Middlesex County, Massachusetts" | NAME == "Watertown Town city, Middlesex County, Massachusetts")
...
# Step 9: We now have to append these special cases to our state census data, redo the join, and run one more check for NA values.
census_data_mn_2021 <- bind_rows(census_data_mn_2021, census_data_zips_mn_2021)
hometowns_mn_complete <- distinct_hometowns_mn %>%
  left_join(census_data_mn_2021, by = c("hometown_city_clean" = "city", "hometown_state_clean" = "state")) %>%
  mutate(players_per_thousand = round((total_players*1000)/total_pop,1)) %>%
  arrange(desc(players_per_thousand)) 
...
# For Greenbank (98253) and Veradale (99037):
...
census_data_wi_subdivisions_2021 <- get_acs(geography = "county subdivision",
                           ...
                           state = "WI",
                           output = "wide") %>%
  filter(NAME == "Erin town, Washington County, WI" | NAME == "Gardner town, Door County, WI")



# Step 9: We now have to append these special cases to our state census data, redo the join, and run one more check for NA values.
census_data_wi_2021 <- bind_rows(census_data_wi_2021, census_data_subdivisions_wi_2021)
hometowns_wi_complete <- distinct_hometowns_wi %>%
  left_join(census_data_wi_2021, by = c("hometown_city_clean" = "city", "hometown_state_clean" = "state")) %>%
  mutate(players_per_thousand = round((total_players*1000)/total_pop,1)) %>%
  arrange(desc(players_per_thousand))
...
# ALL HOMETOWNS
hometowns_complete <- bind_rows(hometowns_al_complete, hometowns_az_complete, hometowns_fl_complete, hometowns_ga_complete, hometowns_hi_complete, hometowns_ia_complete, hometowns_in_complete, hometowns_ky_complete, hometowns_la_complete, hometowns_md_complete, hometowns_mi_complete, hometowns_mo_complete, hometowns_ms_complete, hometowns_nc_complete, hometowns_ne_complete, hometowns_nj_complete, hometowns_oh_complete, hometowns_sc_complete, hometowns_tx_complete, hometowns_ut_complete, hometowns_va_complete, hometowns_small_states_complete) %>%
