...
football_rosters_usa_states <- football_rosters_usa_states %>%
  ...
  left_join(state_crosswalk, by=c("hometown_state_clean" = "state_abb"))
...
census_data_md_2022 <- census_data_md_2022 %>%
  mutate(state = case_when(
    state == 'Maryland' ~ "MD")) %>%
  filter(geoid != "2405525" & geoid != "2416620" & geoid != "2479875") #narrowing down to one option for Bel Air and Chevy Chase and getting rid of this version of Upper Marlboro
...
# We also need to do something about Upper Marlboro: we'll use the county subdivision https://censusreporter.org/profiles/06000US2403390248-district-3-marlboro-prince-georges-county-md/
census_data_md_subdivisions_2022 <- get_acs(geography = "county subdivision",
                           variables = c("B01003_001", "B02001_003", "B19013_001"),
                           year = 2022,
                           state = "MD",
                           output = "wide") %>%
  clean_names() %>%
  filter(name == "District 3, Marlboro, Prince George's County, Maryland") %>%
  rename(city = name, total_pop = b01003_001e, black_pop = b02001_003e, median_income = b19013_001e) %>%
  mutate(pct_black = round(black_pop/total_pop*100, 1)) %>%
  mutate(state = "MD") %>%
  mutate(city = case_when(
    city == "District 3, Marlboro, Prince George's County, Maryland" ~ "Upper Marlboro")) %>%
  select(geoid, city, state, total_pop, black_pop, median_income, pct_black)

# Step 9: We now have to append these special cases to our state census data, redo the join, and run one more check for NA values.
census_data_md_2022 <- bind_rows(census_data_md_2022, census_data_zips_md_2022, census_data_md_subdivisions_2022)
...
three_plus <- hometowns_complete %>%
  filter(total_players >= 3) %>%
  arrange(desc(players_per_thousand))

three_plus_top_50 <- three_plus %>%
  head(50) %>%
  arrange(desc(players_per_thousand))

#write_csv(three_plus, file = "three_plus_players.csv")

#write_csv(three_plus_top_50, file = "three_plus_players_top_50.csv")
...
race_analysis_vs_state <- hometowns_complete_with_race %>%
  ...
  mutate(players_per_hundred_thousand = round((sum_total_players*100000)/sum_total_pop,1)) %>%
