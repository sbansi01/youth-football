...
census_data_zips_ma_2021 <- get_acs(geography = "zcta",
                           variables = c("B01003_001", "B02001_003", "B19013_001"),
                           year = 2021,
                           output = "wide") %>%
  clean_names() %>%
  filter(name == "ZCTA5 02124" | name == "ZCTA5 02136" | name == "ZCTA5 02132" | name == "ZCTA5 02467") %>%
  rename(city = name, total_pop = b01003_001e, black_pop = b02001_003e, median_income = b19013_001e) %>%
  mutate(pct_black = round(black_pop/total_pop*100, 1)) %>%
  mutate(state = "MA") %>%
  mutate(city = case_when(
    city == "ZCTA5 02124" ~ "Dorchester",
    city == "ZCTA5 02136" ~ "Hyde Park",
    city == "ZCTA5 02132" ~ "West Roxbury", 
    city == "ZCTA5 02467" ~ "Chestnut Hill")) %>%
  select(geoid, city, state, total_pop, black_pop, median_income, pct_black)



# Step 9: We now have to append these special cases to our state census data, redo the join, and run one more check for NA values.
census_data_ma_2021 <- bind_rows(census_data_ma_2021, census_data_zips_ma_2021)
...
census_data_ny_subdivisions_2021 <- get_acs(geography = "county subdivision",
                           ...
                           state = "NY",
                           output = "wide") %>%
  filter(NAME == "Somers town, Westchester County, New York")
...
# Step 8: Now we have to figure out how to address each of the NA values. This is a judgment call, and we should be careful about documenting how and why we made that decision. If there is an NA value because a hometown is misspelled, tell Sapna, and she will make that correction in Open Refine. If there is an NA value because the census does not recognize the hometown, we need to find some substitute for that hometown.
 ...
 # For Ballston Lake (12019)

#https://censusreporter.org/profiles/86000US12019-12019/
census_data_zips_ny_2021 <- get_acs(geography = "zcta",
                           variables = c("B01003_001", "B02001_003", "B19013_001"),
                           year = 2021,
                           output = "wide") %>%
  clean_names() %>%
  filter(name == "ZCTA5 12019" | name == "ZCTA5 13078" | name == "ZCTA5 10577") %>% 
  rename(city = name, total_pop = b01003_001e, black_pop = b02001_003e, median_income = b19013_001e) %>%
  mutate(pct_black = round(black_pop/total_pop*100, 1)) %>%
  mutate(state = "NY") %>%
  mutate(city = case_when(
    city == "ZCTA5 12019" ~ "Ballston Lake",
    city == "ZCTA5 13078" ~ "Jamesville",
    city == "ZCTA5 10577" ~ "Purchase"))%>%
  select(geoid, city, state, total_pop, black_pop, median_income, pct_black)
...
# ALL HOMETOWNS
hometowns_complete <- bind_rows(hometowns_al_complete, hometowns_ar_complete, hometowns_az_complete, hometowns_ca_complete, hometowns_fl_complete, hometowns_ga_complete, hometowns_hi_complete, hometowns_ia_complete, hometowns_il_complete, hometowns_in_complete, hometowns_ks_complete, hometowns_ky_complete, hometowns_la_complete, hometowns_md_complete, hometowns_mi_complete, hometowns_mo_complete, hometowns_ms_complete, hometowns_nc_complete, hometowns_ne_complete, hometowns_nj_complete, hometowns_oh_complete, hometowns_ok_complete, hometowns_or_complete, hometowns_sc_complete, hometowns_tn_complete, hometowns_tx_complete, hometowns_ut_complete, hometowns_va_complete, hometowns_wa_complete, hometowns_small_states_complete) %>%
