...
Census_2020 <- load_variables(2020, "pl")
...
# GEORGIA HOMETOWNS - starting with this one and some others that will likely be most consequential for the project.
...
# Step 3: Grab the relevant census data for that state. Note: B01003 = total population, B02009 = total Black population, B19013 = median household income. We get these codes using the ACS crosswalk we loaded earlier (ACS_2001) and this website: https://censusreporter.org/topics/table-codes/
census_data_ga_2021 <- get_acs(geography = "place",
                           variables = c("B01003_001", "B02009_001", "B19013_001"),
                           year = 2021,
                           state = "GA")
...
census_data_ga_2021 <- census_data_ga_2021 %>%
  ...
  select(-name) %>%
  rename(total_pop = B01003_001, black_pop = B02009_001, median_income = B19013_001) %>%
  mutate(pct_black = round(black_pop/total_pop*100, 1))

# Step 5: State-specific cleaning of census data to prepare for join with hometowns/roster data. This part is different for every state. The first mutate function should be included for every state, just modified depending on the state name. The second mutate function is only for instances when the census names something in a weird way that doesn't line up with the hometowns.
census_data_ga_2021 <- census_data_ga_2021 %>%
...
# Step 8: Now we have to figure out how to address each of the NA values. This is a judgment call, and we should be careful about documenting how and why we made that decision.
...
# Georgia, but just hometowns that have produced at least 3 players, are at least 40 percent Black, and median income <$50,000 (used this for reference: https://www.census.gov/quickfacts/fact/table/GA/PST045222)

hometowns_ga_3plusblackicome <- hometowns_ga %>%
  filter(total_players >= 3) %>%
  filter(pct_black >= 40.0) %>%
  filter(median_income <= 50000)

#write_csv(hometowns_ga_3plusblackicome, file = "hometowns_ga_3plus_blackincome.csv")

# Georgia, but just hometowns that have produced at least 5 players
hometowns_ga_5plus <- hometowns_ga %>%
  filter(total_players >= 5)

#write_csv(hometowns_ga_5plus, file = "hometowns_ga_5plus.csv")
